...
import { checkPersonalEmailAvailability, checkUsernameAvailability, getUserIDandType, loginUsertoDatabase, registerUsertoDatabase } from '../services/entry';
        ...
        if (checkerForInput.message['message'] === 'success') {
            ...
            if (loginUpdate['message'] === 'success') {
                ...
                if (userData) {
                    ...
                    loginUpdate = { ...loginUpdate, accessToken: accessToken, userType: userTypeHash, userFullName, username };
                } else {
                    data.code = 400;
...
export const registerUserController = async (req: Request, res: Response) => {
    try {
        const { firstName, middleName, lastName, personalEmail, studentID, username, password, userType } = req.body;
        const checkerForInput = await checkEveryInputForSignup(username, personalEmail, password);
        if (checkerForInput.message['message'] === 'success') {
            const data = await registerUsertoDatabase(firstName, middleName, lastName, username, personalEmail, password, userType, studentID);
